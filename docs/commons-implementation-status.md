...
- Lease engine with observe/read/write/exclusive/maintenance modes, namespaced canonical ids, separator and traversal normalization, local aliases, write-mode fencing epochs, owner-and-epoch-checked remote release, idempotent release, migration conflict blocking, renew, force release, conflict checks, and persisted denial audit events.
